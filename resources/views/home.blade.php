...
<section class="mt-12">
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        @forelse ($bentoCategories as $index => $categoryName)
            ...
            @endphp
            <article class="group relative overflow-hidden rounded-3xl border border-white/10 shadow-lg">
                <img src="{{ $imageUrl }}" alt="{{ $categoryName }}" class="h-56 w-full object-cover transition duration-500 group-hover:scale-105">
                <div class="absolute inset-0 bg-gradient-to-t from-lux-bg/90 via-lux-bg/30 to-transparent"></div>
...
<section class="mt-12">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        @forelse ($featuredProducts as $product)
            <article class="group overflow-hidden rounded-3xl border border-white/5 bg-lux-card shadow-md transition-all duration-500 hover:-translate-y-2 hover:border-lux-gold/30 hover:shadow-2xl">
                <a href="{{ route('products.show', $product) }}" class="relative block overflow-hidden">
                    ...
                    <span class="absolute right-3 top-3 rounded-full bg-lux-bg/80 px-3 py-1 text-xs font-semibold text-lux-gold shadow-sm">{{ number_format($product->price, 0, ',', '.') }} VND</span>
                </a>
                <div class="p-4">
                    <div class="flex items-center justify-between gap-2">
                        <p class="text-xs uppercase tracking-[0.2em] text-lux-text/70">{{ $product->category->name ?? 'Flower' }}</p>
                        <span class="rounded-full bg-emerald-900/60 px-3 py-1 text-xs font-semibold text-lux-gold">Còn hàng</span>
                    </div>
                    <h3 class="mt-2 text-lg font-semibold text-lux-text">{{ $product->name }}</h3>
                    <div class="mt-4 flex items-center justify-between gap-2">
                        ...
                        @auth
                            <form method="POST" action="{{ route('cart.store') }}" class="opacity-0 transition duration-300 group-hover:opacity-100">
                                ...
                                <button type="submit" class="rounded-lg bg-lux-gold px-3 py-2 text-xs font-semibold text-lux-bg shadow-sm transition hover:-translate-y-0.5">
                                    Thêm vào giỏ hàng
                                </button>
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="opacity-0 text-xs font-semibold text-lux-gold transition duration-300 group-hover:opacity-100">Đăng nhập để mua</a>
                        @endauth
                    </div>
                </div>
            </article>
